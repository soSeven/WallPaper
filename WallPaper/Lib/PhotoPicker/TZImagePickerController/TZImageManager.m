...
- (void)yb_getVideoOutputPathWithAsset:(PHAsset *)asset success:(void (^)(NSString *outputPath, NSString *fileName))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    [self yb_getVideoOutputPathWithAsset:asset presetName:AVAssetExportPreset640x480 success:success failure:failure];
}

- (void)yb_getVideoOutputPathWithAsset:(PHAsset *)asset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath, NSString *fileName))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset* avasset, AVAudioMix* audioMix, NSDictionary* info){
        ...
        [self startExportVideoWithVideoAsset:videoAsset presetName:presetName success:^(NSString *outputPath) {
            !success?:success(outputPath, videoAsset.URL.lastPathComponent);
        } failure:failure];
    }];
}
