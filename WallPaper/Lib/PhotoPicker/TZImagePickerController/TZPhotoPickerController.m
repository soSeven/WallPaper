...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setImage:[UIImage tz_imageNamedFromMyBundle:@"photo_original_sel"] forState:UIControlStateSelected];
    ...
    NSString *title = tzImagePickerVc.doneBtnTitleStr;
    if (tzImagePickerVc.selectedModels.count > 0) {
        title = [NSString stringWithFormat:@"%@(%zd)", tzImagePickerVc.doneBtnTitleStr, tzImagePickerVc.selectedModels.count];
    }
...
//    [_doneButton setBackgroundColor:[UIColor colorWithHexString:@"#FF5252"]];
    [_doneButton addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [_doneButton setTitle:title forState:UIControlStateNormal];
    [_doneButton setTitle:title forState:UIControlStateDisabled];
//    [_doneButton setBackgroundImage:[UIImage imageCreatWithColor:[UIColor colorWithHexString:@"#FF5252"]] forState:UIControlStateNormal];
//    [_doneButton setBackgroundImage:[UIImage imageCreatWithColor:[[UIColor colorWithHexString:@"#FF5252"] colorWithAlphaComponent:0.5]] forState:UIControlStateDisabled];
...
//    [_bottomToolBar addSubview:_numberLabel];
    ...
    if (tzImagePickerVc.photoPickerPageUIConfigBlock) {
        tzImagePickerVc.photoPickerPageUIConfigBlock(_collectionView, _bottomToolBar, _previewButton, _originalPhotoButton, _originalPhotoLabel, _doneButton, nil, nil, _divideLine);
    }
}
...
//    _divideLine.frame = CGRectMake(0, 0, self.view.tz_width, 1);
    ...
    CGFloat doneButtonWidth = 73;
    _doneButton.frame = CGRectMake(self.view.tz_width - doneButtonWidth - 12, 9, doneButtonWidth, 32);
    ...
    if (tzImagePickerVc.photoPickerPageDidLayoutSubviewsBlock) {
        tzImagePickerVc.photoPickerPageDidLayoutSubviewsBlock(_collectionView, _bottomToolBar, _previewButton, _originalPhotoButton, _originalPhotoLabel, _doneButton, nil, nil, _divideLine);
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tzImagePickerVc.selectedModels.count >= tzImagePickerVc.maxImagesCount && tzImagePickerVc.showPhotoCannotSelectLayer && !model.isSelected) {
        ...
        if (tzImagePickerVc.showPhotoCanSelectLayer && model.isSelected) {
            cell.cannotSelectLayerButton.backgroundColor = tzImagePickerVc.canSelectLayerColor;
            cell.cannotSelectLayerButton.hidden = NO;
        }
        else {
...
//        __strong typeof(weakLayer) strongLayer = weakLayer;
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.showSelectedIndex || tzImagePickerVc.showPhotoCannotSelectLayer
                || tzImagePickerVc.showPhotoCanSelectLayer) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            if (![TZImageManager manager].yb_videoEditModel) {
                ...
                // by liqi
                // 更改跳转
                YBVideoEditorController *videoeditor = [[YBVideoEditorController alloc] init];
                videoeditor.model = model;
                [self.navigationController pushViewController:videoeditor animated:YES];
            }  
        }
...
//    _numberLabel.text = [NSString stringWithFormat:@"%zd",tzImagePickerVc.selectedModels.count];
    ...
    if (tzImagePickerVc.photoPickerPageDidRefreshStateBlock) {
        tzImagePickerVc.photoPickerPageDidRefreshStateBlock(_collectionView, _bottomToolBar, _previewButton, _originalPhotoButton, _originalPhotoLabel, _doneButton, nil, nil, _divideLine);;
    }
}
