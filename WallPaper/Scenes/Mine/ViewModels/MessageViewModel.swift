...
class MessageViewModel: ViewModel, ViewModelType {
    ...
    struct Output {
        let items: BehaviorRelay<[MessageListModel]>
    ...
    func transform(input: Input) -> Output {
        ...
        let elements = BehaviorRelay<[MessageListModel]>(value: [])
    ...
    func requestList() -> Observable<[MessageListModel]> {
        return NetManager.requestObjArray(.helpList(page: page, limit: limit), type: MessageListModel.self).trackActivity(loading).trackError(error)
    }
