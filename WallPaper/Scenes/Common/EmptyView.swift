...
class EmptyView: UIView {
    ...
    let imgView = UIImageView(image: UIImage(named: "search_img_default"))
    let label: UILabel = {
        let l = UILabel()
        l.text = "呜呜，没有相关内容"
    ...
    let btn: UIButton = {
        ...
        b.backgroundColor = .init(hex: "#FF2071")
    ...
    override init(frame: CGRect) {
        ...
        label.snp.makeConstraints { make in
            make.centerX.equalToSuperview()
            labelTop = make.top.equalTo(imgView.snp.bottom).offset(15.uiX)
        }
